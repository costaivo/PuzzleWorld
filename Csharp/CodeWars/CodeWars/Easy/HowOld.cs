 ...
 * As Philip is getting more courious every day he may soon want to know how many years it was until he would be born, so your function needs to work with both dates in the future and in the past.
...
namespace CodeWars.Easy
{
    public  class HowOld
    {
        public static string CalculateAge(int birth, int yearTo)
        {
            ...
            if(age>0)
            {
                return string.Format("You are {0} year{1} old.", age,age>1? "s":"");
            }
            else if(age<0)
            {
                age = age * -1;
                return string.Format("You will be born in {0} year{1}.", age,age > 1 ? "s" : "");
            }
            else
